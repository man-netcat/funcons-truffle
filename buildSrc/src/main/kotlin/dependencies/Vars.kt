package dependencies

object Vars {
    const val GENERATEDPATHSTR = "../fctlang/src/main/kotlin/generated"
    const val CBSFILEPATH = "../CBS-beta/Funcons-beta/"
    val configFiles = listOf<String>(
//        "Computations/Abnormal/Failing/tests/else-choice.config", //TODO Requires random choice
//        "Computations/Abnormal/Throwing/tests/catch-else-throw.config",
//        "Computations/Abnormal/Throwing/tests/handle-recursively.config",
//        "Computations/Normal/Flowing/tests/atomic.config",
//        "Computations/Normal/Giving/tests/fold-left.config",
//        "Computations/Normal/Giving/tests/fold-right.config",
//        "Computations/Normal/Storing/tests/structural-assign-1.config",
//        "Computations/Normal/Storing/tests/structural-assign-2.config",
//        "Computations/Normal/Storing/tests/structural-assign-3.config",
//        "Computations/Normal/Storing/tests/structural-assigned-1.config",
//        "Computations/Normal/Storing/tests/structural-assigned-2.config",
//        "Computations/Normal/Storing/tests/structural-assigned-3.config",
//        "Computations/Normal/Storing/tests/structural-assigned-4.config",
//        "Computations/Normal/Storing/tests/recycle-variables-2.config",
        "Computations/Abnormal/Abrupting/tests/finally.config",
        "Computations/Abnormal/Abrupting/tests/handle-abrupt.config",
        "Computations/Abnormal/Breaking/tests/handle-break.config",
        "Computations/Abnormal/Continuing/tests/handle-continue.config",
        "Computations/Abnormal/Failing/tests/check-true.config",
        "Computations/Abnormal/Failing/tests/checked.config",
        "Computations/Abnormal/Failing/tests/defined.config",
        "Computations/Abnormal/Failing/tests/else.config",
        "Computations/Abnormal/Returning/tests/handle-return.config",
        "Computations/Abnormal/Throwing/tests/handle-thrown.config",
        "Computations/Normal/Binding/tests/accumulate.config",
        "Computations/Normal/Binding/tests/bind-recursively.config",
        "Computations/Normal/Binding/tests/bind-value.config",
        "Computations/Normal/Binding/tests/bound-directly.config",
        "Computations/Normal/Binding/tests/bound-value.config",
        "Computations/Normal/Binding/tests/closed.config",
        "Computations/Normal/Binding/tests/collateral.config",
        "Computations/Normal/Binding/tests/environments.config",
        "Computations/Normal/Binding/tests/fresh-identifier.config",
        "Computations/Normal/Binding/tests/identifiers.config",
        "Computations/Normal/Binding/tests/initialise-binding.config",
        "Computations/Normal/Binding/tests/recursive.config",
        "Computations/Normal/Binding/tests/scope.config",
        "Computations/Normal/Binding/tests/unbind.config",
        "Computations/Normal/Flowing/tests/choice.config",
        "Computations/Normal/Flowing/tests/do-while.config",
        "Computations/Normal/Flowing/tests/effect.config",
        "Computations/Normal/Flowing/tests/if-true-else.config",
        "Computations/Normal/Flowing/tests/interleave.config",
        "Computations/Normal/Flowing/tests/left-to-right.config",
        "Computations/Normal/Flowing/tests/right-to-left.config",
        "Computations/Normal/Flowing/tests/sequential.config",
        "Computations/Normal/Flowing/tests/while.config",
        "Computations/Normal/Generating/tests/fresh-atom.config",
        "Computations/Normal/Generating/tests/use-atom-not-in.config",
        "Computations/Normal/Giving/tests/give.config",
        "Computations/Normal/Giving/tests/no-given.config",
        "Computations/Normal/Interacting/tests/print-1.config",
        "Computations/Normal/Interacting/tests/print-2.config",
        "Computations/Normal/Interacting/tests/read-1.config",
        "Computations/Normal/Interacting/tests/read-2.config",
        "Computations/Normal/Interacting/tests/read-3.config",
        "Computations/Normal/Linking/tests/follow-if-link.config",
        "Computations/Normal/Linking/tests/follow-link.config",
        "Computations/Normal/Linking/tests/fresh-initialised-link.config",
        "Computations/Normal/Linking/tests/fresh-link.config",
        "Computations/Normal/Linking/tests/initialise-linking.config",
        "Computations/Normal/Linking/tests/links.config",
        "Computations/Normal/Linking/tests/set-link.config",
        "Computations/Normal/Storing/tests/allocate-initialised-variable.config",
        "Computations/Normal/Storing/tests/allocate-variable.config",
        "Computations/Normal/Storing/tests/assign.config",
        "Computations/Normal/Storing/tests/assigned.config",
        "Computations/Normal/Storing/tests/current-value-2.config",
        "Computations/Normal/Storing/tests/current-value.config",
        "Computations/Normal/Storing/tests/initialise-storing.config",
        "Computations/Normal/Storing/tests/initialise-variable.config",
        "Computations/Normal/Storing/tests/locations.config",
        "Computations/Normal/Storing/tests/recycle-variables-1.config",
        "Computations/Normal/Storing/tests/recycle-variables-3.config",
        "Computations/Normal/Storing/tests/store-clear.config",
        "Computations/Normal/Storing/tests/stores.config",
        "Computations/Normal/Storing/tests/un-assign.config",
        "Values/Composite/Classes/tests/class.config",
        "Values/Composite/Lists/tests/lists.config",
        "Values/Composite/Maps/tests/map-delete.config",
        "Values/Composite/Maps/tests/map-domain.config",
        "Values/Composite/Maps/tests/map-elements.config",
        "Values/Composite/Maps/tests/map-lookup.config",
        "Values/Composite/Maps/tests/map-override.config",
        "Values/Composite/Maps/tests/map-unite.config",
        "Values/Composite/Maps/tests/map.config",
        "Values/Composite/Sequences/tests/drop-first-n.config",
        "Values/Composite/Sequences/tests/first-n.config",
        "Values/Composite/Sequences/tests/first.config",
        "Values/Composite/Sequences/tests/index.config",
        "Values/Composite/Sequences/tests/intersperse.config",
        "Values/Composite/Sequences/tests/is-in.config",
        "Values/Composite/Sequences/tests/length.config",
        "Values/Composite/Sequences/tests/n-of.config",
        "Values/Composite/Sequences/tests/reverse.config",
        "Values/Composite/Sequences/tests/second.config",
        "Values/Composite/Sequences/tests/third.config",
        "Values/Composite/Sets/tests/set-difference.config",
        "Values/Composite/Sets/tests/set-elements.config",
        "Values/Composite/Sets/tests/set-unite.config",
        "Values/Composite/Sets/tests/set.config",
        "Values/Composite/Sets/tests/some-element.config",
        "Values/Composite/Trees/tests/forest-branch-sequence.config",
        "Values/Composite/Trees/tests/forest-root-value-sequence.config",
        "Values/Composite/Trees/tests/forest-value-sequence.config",
        "Values/Composite/Trees/tests/single-branching-sequence.config",
        "Values/Composite/Trees/tests/tree-branch-sequence.config",
        "Values/Composite/Trees/tests/tree-root-value.config",
        "Values/Composite/Trees/tests/tree.config",
        "Values/Composite/Tuples/tests/tuple-zip.config",
        "Values/Composite/Vectors/tests/vector.config",
        "Values/Primitive/Booleans/tests/and.config",
        "Values/Primitive/Booleans/tests/exclusive-or.config",
        "Values/Primitive/Booleans/tests/implies.config",
        "Values/Primitive/Booleans/tests/not.config",
        "Values/Primitive/Booleans/tests/or.config",
        "Values/Value-Types/tests/cast-to-type.config",
        "Values/Value-Types/tests/is-equal.config",
        "Values/Value-Types/tests/is-in-type.config",
        "Values/Value-Types/tests/is-value.config",
        "Values/Value-Types/tests/when-true.config",
    ).map { "$CBSFILEPATH$it" }.toTypedArray()
}
